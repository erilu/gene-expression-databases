...
The RNA-seq portal provides the same data, but on more cell types and using a more sensitive gene expression assessment:

[Image]:

We can also plug in a gene signature of interest and see which cell types express the highest amounts of each gene in the set:

[Image]

What is nice about this website is that it provides the flow cytometry gating strategy and purity for each of the sorted cell subsets.

## 3. Single-cell gene expression databases

There are a few good single-cell RNA-seq databases. Single cell RNA-seq is useful for determining the expression of a gene within a very heterogenous mixture of cells. This is particularly useful for determining the specificity of a gene of interest for a cell type that you wish to target, and for generating cell-type specific gene signatures. 
