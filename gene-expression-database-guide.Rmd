...
```{r echo = F}
include_graphics("images/biogps_p2ry8_cell_atlas.png")
```
...
```{r echo = F}
include_graphics("images/biogps_change_dataset.png")
```
...
```{r echo = F}
include_graphics("images/biogps_p2ry8_tissue.png")
```
...
```{r echo = F}
include_graphics("images/gtex_p2ry8_tissue.png")
```
...
```{r echo = F}
include_graphics("images/hpa_p2ry8_result.png")
```
...
```{r echo = F}
include_graphics("images/hpa_p2ry8_tonsil.png")
```
...
```{r echo = F}
include_graphics("images/hpa_p2ry8_cell_line.png")
```
...
```{r echo = F}
include_graphics("images/immgen_gpr183_microarray.png")
```

The RNA-seq portal provides similar data, but on more cell types. RNA-seq is typically a more robust method of assessing gene expression, but I would recommend searching your gene on both portals to see if the results match. What is nice about this website is that clicking on the cell subset will display the flow cytometry gating strategy and purity. For example, clicking on the "DC.8+.Sp" subset will open a PDF that displays how the cells were stained and gated:

```{r echo = F}
include_graphics("images/immgen_sort_gating.png")
```
...
```{r echo = F}
include_graphics("images/ebi_sc_databases.png")
```
...
```{r echo = F}
include_graphics("images/ebi_headneck_cd19_result.png")
```
...
```{r echo = F}
include_graphics("images/ebi_headneck_markers.png")
```
...
```{r echo = F}
include_graphics("images/broadsc_vlnplot.png")
```
...
```{r echo = F}
include_graphics("images/broadsc_scatterplot.png")
```
...
```{r echo = F}
include_graphics("images/firebrowse_gpr183.png")
```
...
```{r echo = F}
include_graphics("images/oncolnc_gpr183_cox.png")
```
...
```{r echo = F}
include_graphics("images/oncolnc_gpr183_skcm.png")
```
...
Another survival analysis tool that allows for more customization than the above is Gepia2, at http://gepia2.cancer-pku.cn/#index. To perform a survival analysis, navigate to the survival tab: http://gepia2.cancer-pku.cn/#survival. Then, input the gene of interest, and select the indications you would like to perform the analysis on. You can also customize the type of split, error bars, etc. This tool is a bit more versatile than the tool at oncolnc.org. Below is the same 50:50 split on SKCM for GPR183, using Gepia2:

```{r echo = F}
include_graphics("images/gepia2_settings.png")
include_graphics("images/gepia2_results.png")
```
